...
#---------------A2DP METHODS-------------------------------
    ...
    def build_a2dp_source_tab(self):
        """
        Create and return the A2DP source tab UI.
        ...
        streaming_label.setStyleSheet("color:white;")
        ...
        '''
        #device_label = QLabel(f"Streaming to Device: {self.device_address}")
        #device_label.setFont(bold_font)
        #device_label.setStyleSheet("color:black;")
        #layout.addWidget(device_label)

        # Store the address for streaming
        self.device_address_source = self.device_address
        print(self.device_address_source)
        ...
        self.start_streaming_button.setStyleSheet("color:black;")
        ...
        self.stop_streaming_button.setStyleSheet("color:black;")
    ...
    '''
    def load_a2dp_ui_for_device(self, device_address):
        bold_font = QFont()
        bold_font.setBold(True)
        ...
        self.profile_description_text_browser.append(f"A2DP Profile for {device_address}")
        ...
        self.a2dp_tab_widget.setStyleSheet("color:black; background-color: lightblue;")
    ...
    def build_a2dp_sink_tab(self):
        """
        Create and return the A2DP sink tab UI.
        ...
        # Store the address for streaming
        self.device_address_sink = self.device_address

        # Media Controls
        control_label = QLabel("Media Control:")
        control_label.setFont(bold_font)
        control_label.setStyleSheet("color:black;")
    ...
    def on_device_selected_for_a2dp(self):
        """
        Handle selection change for A2DP source device.
    ...
    def on_device_selected_for_a2dp_sink(self):
        """
        Handle selection change for A2DP sink device.
...
#---------PROFILE TAB SELECTION-----------------------------
    def on_profile_tab_changed(self, index):
        ...
        if selected_tab == "A2DP":
            ...
            layout = QVBoxLayout()
            role = self.get_a2dp_role_for_device(self.device_address)
            ...
            if role == "sink":
                tab = self.build_a2dp_source_tab()
                tab.setStyleSheet("color: black;")
                self.a2dp_tab_widget.addTab(tab, "A2DP Source")
            elif role == "source":
                tab = self.build_a2dp_sink_tab()
                tab.setStyleSheet("color: black;")
                self.a2dp_tab_widget.addTab(tab, "A2DP Sink")
            elif role == "both":
                tab1 = self.build_a2dp_source_tab()
                tab1.setStyleSheet("color: black;")
                tab2 = self.build_a2dp_sink_tab()
                tab2.setStyleSheet("color: black;")
                self.a2dp_tab_widget.addTab(tab1, "A2DP Source")
                self.a2dp_tab_widget.addTab(tab2, "A2DP Sink")

            layout.addWidget(self.a2dp_tab_widget)
